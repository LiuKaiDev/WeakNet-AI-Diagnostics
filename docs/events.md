...
WeakNet 事件系统为网络状态变化提供事件监听和通知机制。当网卡变化或默认上网网卡发生改变时，server 会发送相应事件给 client，client 通过监听接口接收这些事件。
...
```bash
# 获取支持的事件类型
./client/bin/test-client event-types

# 订阅特定事件
./client/bin/test-client event-sub InterfaceChanged

# 检查事件（非阻塞）
./client/bin/test-client events
```
...
# 订阅连接方式变化事件
./client/bin/test-client event-sub ConnectionModeChanged
```
